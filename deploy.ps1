...
wsl -u root bash -c "cd ${WslPath} && docker run --rm -v '${WslPath}:/app' -w /app rust:alpine sh -c 'apk add --no-cache musl-dev openssl-dev pkgconfig && cargo build --release'"
...
$remoteCmd = "cd $RemoteDir; fuser -k 18400/tcp 2>/dev/null || true; docker compose down 2>/dev/null || true; sed -i 's/change-me-to-32-byte-secret-key!!/$Key/' docker-compose.yaml; docker load -i transparentllm.tar; docker compose up -d; sleep 3; curl -so /dev/null -w '%{http_code}' http://127.0.0.1:18400/ui/; echo ' /ui/'"

ssh $Server $remoteCmd

Write-Host "`n部署完成! http://8.137.187.63:18400/ui/" -ForegroundColor Green
