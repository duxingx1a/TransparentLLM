...
export interface AreaProps {
  ...
  dataKey: string;
  /** Key in data for the lower bound of stacked area. When set, area fills from baseDataKey to dataKey. */
  baseDataKey?: string;
...
export function Area({
  ...
  // 堆叠模式：y0=底部累计值，y1=顶部累计值
  const getY0 = useCallback(
    (d: Record<string, unknown>) => {
      if (!baseDataKey) return 0;
      const value = d[baseDataKey];
      return typeof value === "number" ? (yScale(value) ?? 0) : 0;
    },
    [baseDataKey, yScale]
  );
    ...
    <>
      {showSeriesContent && showAreaFill ? (
        <AreaClosed
          ...
          y={getY}
          y0={baseDataKey ? getY0 : undefined}
          yScale={yScale}
        />
      ) : null}
